...
import { abilityKeys, abilityLabels, ancestries, classes, classPowerModifier, classResourceCapacity, mageSlots, modifier, packs, proficiency, regions, scoreCosts, type AbilityKey } from "@/lib/character-data";
...
import { clericPrayers, hunterTechniques, inventorDevices, loadoutSelectionLimit, mageSpells, type LoadoutPower } from "@/lib/class-loadouts";
import { patronBonds, patronDisplayName, patronDomains, patronSources } from "@/lib/patrons";
...
const initialState: CharacterState = {
  ...
  classId: "", specialization: "", classSkills: [], cantrips: [], knownSpells: [], preparedSpells: [], prayers: [], devices: [], hunterTechniques: [], pack: "Gezgin",
  patronId: "", patronName: "", patronDomain: "", patronBond: "",
...
export function CharacterBuilder() {
  ...
  const needsPatron = character.ancestry === "havari" || ["ruhban", "antlasmali"].includes(character.classId);
  const selectedPatron = patronSources.find((item) => item.id === character.patronId);
  const availablePatronDomains = selectedPatron ? patronDomains.filter((domain) => selectedPatron.domains.includes(domain.id)) : patronDomains;
  const patronComplete = !needsPatron || Boolean(character.patronId && character.patronDomain && character.patronBond && (character.patronId !== "ozgun" || character.patronName.trim()));
  ...
  const loadoutLimit = selectedClass ? loadoutSelectionLimit(selectedClass.id, character.level, modifier(character.abilities.zeka)) : { known: 0, prepared: 0 };
  const availableMageSpells = mageSpells.filter((spell) => spell.level <= character.level);
  ...
  const resource = useMemo(() => {
    ...
    const count = classResourceCapacity(selectedClass.id, selectedClass.resourceBase, character.level);
  ...
  const validSteps = [
    ...
    pointsLeft === 0,
    Boolean(character.ancestry && character.region && character.regionSkill && character.regionGift && (character.ancestry !== "mutant" || character.region === "zaun")),
    Boolean(selectedClass && character.classSkills.length === selectedClass.skillCount && (selectedClass.id !== "buyucu" || (character.cantrips.length === 3 && character.knownSpells.length === loadoutLimit.known && character.preparedSpells.length === Math.min(loadoutLimit.prepared, loadoutLimit.known))) && (selectedClass.id !== "ruhban" || character.prayers.length === loadoutLimit.known) && (selectedClass.id !== "mucit" || character.devices.length === loadoutLimit.known) && (selectedClass.id !== "avci" || character.hunterTechniques.length === loadoutLimit.known) && (character.level < 3 || character.specialization) && patronComplete),
  ...
  function chooseClass(id: string) { update("classId", id); update("classSkills", []); update("cantrips", []); update("knownSpells", []); update("preparedSpells", []); update("prayers", []); update("devices", []); update("hunterTechniques", []); update("specialization", ""); const choice = classes.find((item) => item.id === id); if (choice) update("pack", choice.name === "Düzenbaz" ? "Hırsız" : choice.name === "Avcı" ? "Kâşif/Avcı" : choice.name === "Büyücü" ? "Bilgin" : choice.name === "Ruhban" ? "Şifacı" : choice.name === "Mucit" ? "Mucit" : choice.name === "Ozan" ? "Gösteri" : choice.name === "Antlaşmalı" || choice.name === "Dövüş Ustası" ? "Mistik" : choice.name === "Yeminli" ? "Diplomat" : choice.name === "Çağırıcı" ? "Çağırıcı" : choice.name === "Şekil Değiştirici" || choice.name === "Barbar" ? "Kâşif/Avcı" : "Gezgin"); }
  ...
  function toggleLoadout(key: "knownSpells" | "prayers" | "devices" | "hunterTechniques", id: string, limit: number) {
    const values = character[key]; const has = values.includes(id);
    if (has) { update(key, values.filter((entry) => entry !== id)); if (key === "knownSpells") update("preparedSpells", character.preparedSpells.filter((entry) => entry !== id)); }
    else if (values.length < limit) update(key, [...values, id]);
  }
  function togglePreparedSpell(id: string) { const has = character.preparedSpells.includes(id); const maximum = Math.min(loadoutLimit.prepared, character.knownSpells.length); if (has) update("preparedSpells", character.preparedSpells.filter((entry) => entry !== id)); else if (character.preparedSpells.length < maximum && character.knownSpells.includes(id)) update("preparedSpells", [...character.preparedSpells, id]); }
    ...
    <div className="builder-page shell">
      ...
      <div className="builder-layout">
        <section className="builder-panel">
          ...
          {step === 2 && <div className="form-grid"><div className="field full"><span>Soy</span><div className="choice-grid">{ancestries.map((item) => <button type="button" className={`choice-card ${character.ancestry === item.id ? "selected" : ""}`} onClick={() => { update("ancestry", item.id); update("ancestryChoice", ""); if (item.id === "mutant") { update("region", "zaun"); update("regionSkill", ""); update("regionGift", ""); } }} key={item.id}><small>{item.size} · {item.speed} m</small><h3>{item.name}</h3><p><b>{item.trait}:</b> {item.description}</p></button>)}</div></div>{character.ancestry === "vastaya" && <div className="field full"><label>Keskin duyu</label><select value={character.ancestryChoice} onChange={(e) => update("ancestryChoice", e.target.value)}><option value="">Duyu seç…</option><option>Görme</option><option>İşitme</option><option>Koku</option></select></div>}<div className="field full"><span>Bölge</span><div className="choice-grid">{regions.map((item) => <button type="button" disabled={character.ancestry === "mutant" && item.id !== "zaun"} title={character.ancestry === "mutant" && item.id !== "zaun" ? "Mutant kökeni Zaun bölgesine özeldir." : undefined} style={{ "--region-color": item.color } as React.CSSProperties} className={`choice-card region-card ${character.region === item.id ? "selected" : ""}`} onClick={() => { if (character.ancestry === "mutant" && item.id !== "zaun") return; update("region", item.id); update("regionSkill", ""); update("regionGift", ""); }} key={item.id}><small>{item.trait}</small><h3>{item.name}</h3><p>{item.description}</p><div className="tag-row">{item.skills.map((skill) => <span className="tag" key={skill}>{skill}</span>)}</div></button>)}</div></div>{selectedRegion && <><div className="field"><label>Bölge becerisi</label><select value={character.regionSkill} onChange={(e) => update("regionSkill", e.target.value)}><option value="">Beceri seç…</option>{selectedRegion.skills.map((skill) => <option key={skill}>{skill}</option>)}</select></div><div className="field"><label>Bağlantı veya eşya</label><select value={character.regionGift} onChange={(e) => update("regionGift", e.target.value)}><option value="">Seçim yap…</option>{selectedRegion.gifts.map((gift) => <option key={gift}>{gift}</option>)}</select></div></>}</div>}

          {step === 3 && <div className="form-grid"><div className="field full"><span>Sınıf</span><div className="choice-grid">{classes.map((item) => <button type="button" className={`choice-card ${character.classId === item.id ? "selected" : ""}`} onClick={() => chooseClass(item.id)} key={item.id}><small>d{item.hitDie} Can Zarı · {item.resource}</small><h3>{item.name}</h3><p>{item.role}. Ana yetenek: {item.primary}.</p><div className="tag-row"><span className="tag">{item.armor}</span>{item.saves.map((save) => <span className="tag" key={save}>{save} kurtarma</span>)}</div></button>)}</div></div>{selectedClass && <><div className="field full"><span>Sınıf becerileri · {character.classSkills.length}/{selectedClass.skillCount}</span><div className="skill-grid">{selectedClass.skills.map((skill) => { const disabled = !character.classSkills.includes(skill) && character.classSkills.length >= selectedClass.skillCount; return <label className="skill-option" key={skill}><input type="checkbox" checked={character.classSkills.includes(skill)} disabled={disabled} onChange={() => toggleSkill(skill)} /><span>{skill}</span></label>; })}</div></div>{selectedClass.id === "buyucu" && <div className="field full cantrip-selection"><span>Cantripler · {character.cantrips.length}/3</span><p className="helper">Yuva harcamayan üç başlangıç büyüsü seç.</p><div className="choice-grid cantrip-grid">{cantrips.map((cantrip) => { const selected = character.cantrips.includes(cantrip.id); const disabled = !selected && character.cantrips.length >= 3; return <button type="button" disabled={disabled} className={`choice-card ${selected ? "selected" : ""}`} onClick={() => toggleCantrip(cantrip.id)} key={cantrip.id}><small>{cantrip.action} · {cantrip.range}</small><h3>{cantrip.name}</h3><p>{cantrip.effect.replace("{DAMAGE}", cantrip.damageDie ? `1d${cantrip.damageDie.size}` : "")}</p></button>; })}</div></div>}
{selectedClass.id === "buyucu" && <>
  <LoadoutPicker title="Büyü kitabı" helper="Başlangıçta altı büyü seç; sonraki Büyücü seviyelerinde kitap genişler." powers={availableMageSpells} selected={character.knownSpells} limit={loadoutLimit.known} onToggle={(id) => toggleLoadout("knownSpells", id, loadoutLimit.known)} />
  <LoadoutPicker title="Hazırlanan büyüler" helper="Uzun mola sonunda kullanıma hazır büyülerini kitabından seç." powers={availableMageSpells.filter((spell) => character.knownSpells.includes(spell.id))} selected={character.preparedSpells} limit={Math.min(loadoutLimit.prepared, character.knownSpells.length)} onToggle={togglePreparedSpell} />
</>}
{selectedClass.id === "ruhban" && <LoadoutPicker title="Dualar" helper="Lütuf harcayarak kullanacağın duaları seç." powers={clericPrayers} selected={character.prayers} limit={loadoutLimit.known} onToggle={(id) => toggleLoadout("prayers", id, loadoutLimit.known)} />}
{selectedClass.id === "mucit" && <LoadoutPicker title="Cihaz şemaları" helper="Yük harcayarak kullanacağın cihazları seç." powers={inventorDevices} selected={character.devices} limit={loadoutLimit.known} onToggle={(id) => toggleLoadout("devices", id, loadoutLimit.known)} />}
{selectedClass.id === "avci" && character.level >= 2 && <LoadoutPicker title="Avcı teknikleri" helper="Aynı tetikleyiciye yalnızca bir teknik uygulanır." powers={hunterTechniques} selected={character.hunterTechniques} limit={loadoutLimit.known} onToggle={(id) => toggleLoadout("hunterTechniques", id, loadoutLimit.known)} />}
<div className="field"><label>Başlangıç paketi</label><select value={character.pack} onChange={(e) => update("pack", e.target.value)}>{packs.map((pack) => <option key={pack}>{pack}</option>)}</select></div><div className="field"><label>Uzmanlık</label>{character.level >= 3 ? <select value={character.specialization} onChange={(e) => update("specialization", e.target.value)}><option value="">Uzmanlık seç…</option>{selectedClass.specializations.map((spec) => <option key={spec}>{spec}</option>)}</select> : <input readOnly value="3. seviyede seçilecek" />}</div><div className="field full"><span className="helper">Başlangıç: {selectedClass.equipment}.</span></div></>}{needsPatron && <div className="field full patron-picker"><span>İnanç ve Aşkın Bağ</span><p className="helper">{character.ancestry === "havari" && character.classId === "ruhban" ? "Havari kökenin ve Ruhban Lütfun aynı kaynağa bağlanır; ikinci bir güç havuzu kazanmazsın." : character.ancestry === "havari" && character.classId === "antlasmali" ? "Havari kökenin ve Antlaşman varsayılan olarak aynı kaynağa bağlanır; ek Mühür kazanmazsın." : character.classId === "antlasmali" ? "Pakt yaptığın varlığı, antlaşmanın alanını ve bu güçle ilişkini belirle." : character.ancestry === "havari" ? "Aşkın Yankı özelliğinin kaynağını, alanını ve bu varlıkla ilişkini belirle." : "Lütfunun geldiği ilahı, ruhu, yemini veya ideali belirle."}</p><div className="form-grid"><div className="field"><label>{character.classId === "antlasmali" ? "Pakt kaynağı" : "İlah / aşkın kaynak"}</label><select value={character.patronId} onChange={(e) => { const patronId = e.target.value; const source = patronSources.find((item) => item.id === patronId); setCharacter((current) => ({ ...current, patronId, patronName: patronId === "ozgun" ? current.patronName : "", patronDomain: source?.domains.includes(current.patronDomain) ? current.patronDomain : source?.domains[0] ?? "" })); }}><option value="">Kaynak seç…</option>{patronSources.map((source) => <option value={source.id} key={source.id}>{source.name} · {source.kind}</option>)}</select></div>{character.patronId === "ozgun" && <div className="field"><label>Özgün varlığın adı</label><input value={character.patronName} onChange={(e) => update("patronName", e.target.value)} placeholder="GM ile belirlenen ad" /></div>}<div className="field"><label>Alan</label><select value={character.patronDomain} onChange={(e) => update("patronDomain", e.target.value)} disabled={!character.patronId}><option value="">Alan seç…</option>{availablePatronDomains.map((domain) => <option value={domain.id} key={domain.id}>{domain.name}</option>)}</select></div><div className="field"><label>Bağın biçimi</label><select value={character.patronBond} onChange={(e) => update("patronBond", e.target.value)}><option value="">İlişki seç…</option>{patronBonds.map((bond) => <option key={bond}>{bond}</option>)}</select></div></div>{selectedPatron && <span className="helper"><b>{selectedPatron.name}:</b> {selectedPatron.description} Bu seçim yetenek ağacında tematik önerileri öne çıkarır; tek başına büyü veya ek kaynak vermez.</span>}</div>}</div>}
...
function LoadoutPicker({ title, helper, powers, selected, limit, onToggle }: { title: string; helper: string; powers: LoadoutPower[]; selected: string[]; limit: number; onToggle: (id: string) => void }) {
  return <div className="field full cantrip-selection"><span>{title} · {selected.length}/{limit}</span><p className="helper">{helper}</p><div className="choice-grid cantrip-grid">{powers.map((power) => { const chosen = selected.includes(power.id); const disabled = !chosen && selected.length >= limit; return <button type="button" disabled={disabled} className={`choice-card ${chosen ? "selected" : ""}`} onClick={() => onToggle(power.id)} key={power.id}><small>{power.action} · {power.range}</small><h3>{power.name}</h3><p>{power.effect}</p><div className="tag-row"><span className="tag">{power.cost}</span></div></button>; })}</div></div>;
}