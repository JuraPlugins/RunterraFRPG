...
import { abilityKeys, abilityLabels, classes, mageSlots, modifier, proficiency, type AbilityKey } from "@/lib/character-data";
...
type ActionSource = { id: string; name: string; effect: string; kind: "Yetenek" | "Cantrip" | "Sınıf" | "Eşya"; cost?: string; tier?: string; cooldown?: Cooldown | "none"; item?: ItemEntry };

const fallbackAbilities = { guc: 8, ceviklik: 8, dayaniklilik: 8, zeka: 8, sezgi: 8, karizma: 8 };
const defaultCharacter: CharacterRecord = { id: "", name: "", concept: "", level: 1, classId: "", specialization: "", ancestry: "", region: "", abilities: fallbackAbilities, classSkills: [], cantrips: [], regionSkill: "", regionGift: "", pack: "Gezgin", aspects: { identity: "", region: "", conflict: "" } };
...
export function CharacterDashboard({ abilities, items, classFeatures }: { abilities: AbilityEntry[]; items: ItemEntry[]; classFeatures: Record<string, ClassFeature[]> }) {
  ...
  const resourceMax = selectedClass ? selectedClass.id === "mucit" ? prof + 2 : selectedClass.id === "ruhban" && character.level >= 9 ? prof + 1 : prof : 0;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const base: RuntimeState = { currentHp: loadedMax, tempHp: 0, rp: 2, resource: chosen?.id === "mucit" ? proficiency(loaded.level) + 2 : proficiency(loaded.level), spellSlots: [...mageSlots[loaded.level - 1]], conditions: [], deathSuccess: 0, deathFail: 0, inventory: [], unlocked: [], prepared: [], asiLevels: [], cooldowns: {}, inCombat: false, round: 1, turn: 1, actionLog: [], notes: "", quest: "", gold: 15, rolls: [], loadoutMigrated: true, maxHpValue: loadedMax, levelHistory: [], itemUses: {} };
  ...
  const branchAbilities = useMemo(() => {
    ...
    return branches.map((branch) => {
      const scored = abilities.map((ability) => {
        ...
        const score = branch.keywords.reduce((total, keyword) => total + (text.includes(keyword) ? 1 : 0), 0) + (ability.classes.toLocaleLowerCase("tr-TR").includes(selectedClass?.name.toLocaleLowerCase("tr-TR") ?? "---") ? 3 : 0);
        return { ability, score, text };
      }).filter(({ score, text }) => score > 0 && (!search || text.includes(search))).sort((a, b) => b.score - a.score || tierFromText(a.ability.tier) - tierFromText(b.ability.tier));
  ...
  }, [abilities, abilitySearch, character.classId, selectedClass?.name]);
  ...
  function confirmLevelUp() {
    ...
    const nextProf = proficiency(next); const nextResourceMax = activeClass.id === "mucit" ? nextProf + 2 : activeClass.id === "ruhban" && next >= 9 ? nextProf + 1 : nextProf;
  ...
  function useAction(action: ActionSource) {
    ...
    if (action.kind === "Yetenek" && character.classId === "buyucu") {
      ...
      nextSlots[slot] -= 1;
    } else if (/sınıf kaynağı|efor|momentum|odak|lütuf|yük/.test(text)) {
      const count = Number(text.match(/(\d+)\s*(?:sınıf kaynağı|efor|momentum|odak|lütuf|yük)/)?.[1] ?? 1);
