...
import { clericPrayers, hunterMark, hunterTechniques, inventorDevices, loadoutSelectionLimit, mageSpells, type LoadoutPower } from "@/lib/class-loadouts";
...
type CharacterRecord = {
  ...
  abilities: Record<AbilityKey, number>; classSkills: string[]; cantrips: string[]; knownSpells: string[]; preparedSpells: string[]; prayers: string[]; devices: string[]; hunterTechniques: string[]; regionSkill: string; regionGift: string; pack: string;
...
type LevelSnapshot = { level: number; abilities: Record<AbilityKey, number>; currentHp: number; maxHpValue: number; resource: number; spellSlots: number[]; asiLevels: number[]; recoveryDice: number };
type RuntimeState = {
  ...
  recoveryDice: number; woundSeverity: "" | "Hafif" | "Ağır" | "Kritik"; woundAspect: string; woundCreatedThisCombat: boolean; mageRecoveryUsed: boolean; inventorRecoverySucceeded: boolean;
};
type ActionSource = { id: string; name: string; effect: string; kind: "Yetenek" | "Cantrip" | "Sınıf" | "Soy" | "Eşya" | "Büyü" | "Dua" | "Cihaz" | "Teknik"; cost?: string; tier?: string; slotLevel?: number; cooldown?: Cooldown | "none"; item?: ItemEntry };

const fallbackAbilities = { guc: 8, ceviklik: 8, dayaniklilik: 8, zeka: 8, sezgi: 8, karizma: 8 };
const defaultCharacter: CharacterRecord = { id: "", name: "", concept: "", level: 1, classId: "", specialization: "", ancestry: "", region: "", abilities: fallbackAbilities, classSkills: [], cantrips: [], knownSpells: [], preparedSpells: [], prayers: [], devices: [], hunterTechniques: [], regionSkill: "", regionGift: "", pack: "Gezgin", patronId: "", patronName: "", patronDomain: "", patronBond: "", aspects: { identity: "", region: "", conflict: "" } };
const statusOptions = ["Kör", "Büyülenmiş", "Sağır", "Korkmuş", "Yere Düşmüş", "Kısıtlanmış", "Sersemlemiş", "Zehirlenmiş", "Görünmez"];
const tabs = [
...
function normalizeClassLoadout(record: CharacterRecord) {
  const limit = loadoutSelectionLimit(record.classId, record.level, modifier(record.abilities.zeka));
  const atLevel = (powers: LoadoutPower[]) => powers.filter((power) => power.level <= record.level);
  const normalized = { ...record };
  if (record.classId === "buyucu") {
    const available = atLevel(mageSpells);
    normalized.knownSpells = record.knownSpells?.length ? record.knownSpells.filter((id) => available.some((power) => power.id === id)) : available.slice(0, limit.known).map((power) => power.id);
    normalized.preparedSpells = record.preparedSpells?.length ? record.preparedSpells.filter((id) => normalized.knownSpells.includes(id)).slice(0, limit.prepared) : normalized.knownSpells.slice(0, Math.min(limit.prepared, normalized.knownSpells.length));
  }
  if (record.classId === "ruhban" && !record.prayers?.length) normalized.prayers = atLevel(clericPrayers).slice(0, limit.known).map((power) => power.id);
  if (record.classId === "mucit" && !record.devices?.length) normalized.devices = atLevel(inventorDevices).slice(0, limit.known).map((power) => power.id);
  if (record.classId === "avci" && !record.hunterTechniques?.length) normalized.hunterTechniques = atLevel(hunterTechniques).slice(0, limit.known).map((power) => power.id);
  return normalized;
}
export function CharacterDashboard({ abilities, items, classFeatures }: { abilities: AbilityEntry[]; items: ItemEntry[]; classFeatures: Record<string, ClassFeature[]> }) {
  ...
  const [restDraft, setRestDraft] = useState<{ dice: number; slotRecovery: number[] } | null>(null);
  ...
  useEffect(() => {
    ...
    try {
      const loaded = normalizeClassLoadout({ ...defaultCharacter, ...JSON.parse(saved) } as CharacterRecord); loaded.id ||= newCharacterId();
      setCharacter(loaded);
      ...
      const base: RuntimeState = { currentHp: loadedMax, tempHp: 0, rp: 2, resource: chosen ? classResourceCapacity(chosen.id, chosen.resourceBase, loaded.level) : 0, spellSlots: [...mageSlots[loaded.level - 1]], conditions: [], deathSuccess: 0, deathFail: 0, inventory: [], unlocked: [], prepared: [], asiLevels: [], cooldowns: {}, inCombat: false, round: 1, turn: 1, actionLog: [], notes: "", quest: "", gold: 15, rolls: [], loadoutMigrated: true, maxHpValue: loadedMax, levelHistory: [], itemUses: {}, recoveryDice: loaded.level, woundSeverity: "", woundAspect: "", woundCreatedThisCombat: false, mageRecoveryUsed: false, inventorRecoverySucceeded: false };
      const migratedPrepared = old.loadoutMigrated ? (old.prepared ?? []) : (old.prepared?.length ? old.prepared : (old.unlocked ?? []).slice(0, 8));
      setRuntime({ ...base, ...old, prepared: migratedPrepared, loadoutMigrated: true, maxHpValue: old.maxHpValue ?? loadedMax, levelHistory: old.levelHistory ?? [], itemUses: old.itemUses ?? {}, cooldowns: old.cooldowns ?? {}, actionLog: old.actionLog ?? [], round: old.round ?? 1, turn: old.turn ?? 1, recoveryDice: old.recoveryDice ?? loaded.level, woundSeverity: old.woundSeverity ?? "", woundAspect: old.woundAspect ?? "", woundCreatedThisCombat: old.woundCreatedThisCombat ?? false, mageRecoveryUsed: old.mageRecoveryUsed ?? false, inventorRecoverySucceeded: old.inventorRecoverySucceeded ?? false });
...
const branchAbilities = useMemo(() => {
  ...
  const selectedLoadoutPowers: LoadoutPower[] = character.classId === "buyucu" ? mageSpells.filter((power) => character.preparedSpells.includes(power.id))
    : character.classId === "ruhban" ? clericPrayers.filter((power) => character.prayers.includes(power.id))
    : character.classId === "mucit" ? inventorDevices.filter((power) => character.devices.includes(power.id))
    : character.classId === "avci" ? [hunterMark, ...hunterTechniques.filter((power) => character.hunterTechniques.includes(power.id))]
    : [];
  ...
  function adjust(key: "currentHp" | "tempHp" | "rp" | "resource" | "gold", delta: number, min = 0, max = Infinity) {
    if (key === "currentHp") { adjustHp(delta); return; }
    setRun(key, Math.max(min, Math.min(max, activeRuntime[key] + delta)));
  }
  function adjustHp(delta: number) {
    const nextHp = Math.max(0, Math.min(maxHp, activeRuntime.currentHp + delta));
    const returnsFromZero = activeRuntime.currentHp === 0 && nextHp > 0 && !activeRuntime.woundCreatedThisCombat;
    if (!returnsFromZero) { setRun("currentHp", nextHp); return; }
    const severity = activeRuntime.woundSeverity === "" ? "Hafif" : activeRuntime.woundSeverity === "Hafif" ? "Ağır" : "Kritik";
    const aspect = `${severity} Yara — ayrıntısını yaz`;
    setRuntime({ ...activeRuntime, currentHp: nextHp, deathSuccess: 0, deathFail: 0, woundSeverity: severity, woundAspect: aspect, woundCreatedThisCombat: true, actionLog: [`Yara kazanıldı: ${aspect}`, ...activeRuntime.actionLog].slice(0, 20) });
    setLastUse({ id: `wound-${Date.now()}`, name: `${severity} Yara`, result: "0 CP'den döndüğün için bir Yara Aspect'i kazandın. Masa bölümünden düzenleyebilirsin." });
  }
  ...
  function openShortRest() { setRestDraft({ dice: 0, slotRecovery: Array(9).fill(0) }); }
  function changeSlotRecovery(index: number, delta: number) {
    if (!restDraft) return;
    const recovered = [...restDraft.slotRecovery];
    const expended = Math.max(0, (slotsMax[index] ?? 0) - (activeRuntime.spellSlots[index] ?? 0));
    recovered[index] = Math.max(0, Math.min(expended, (recovered[index] ?? 0) + delta));
    const budget = Math.max(1, Math.ceil(character.level / 2));
    const cost = recovered.reduce((sum, amount, slot) => sum + amount * (slot + 1), 0);
    if (cost <= budget) setRestDraft({ ...restDraft, slotRecovery: recovered });
  }
  function confirmShortRest() {
    if (!restDraft) return;
    const spentDice = Math.min(activeRuntime.recoveryDice, restDraft.dice);
    const rolls = Array.from({ length: spentDice }, () => randomDie(activeClass.hitDie));
    const healing = Math.max(0, rolls.reduce((sum, value) => sum + value, 0) + spentDice * conMod);
    let nextResource = activeRuntime.resource;
    let inventorRecoverySucceeded = activeRuntime.inventorRecoverySucceeded;
    let recoveryNote = "";
    if (["savasci", "dovus-ustasi", "antlasmali"].includes(character.classId)) nextResource = resourceMax;
    else if (character.classId === "avci") nextResource = Math.min(resourceMax, nextResource + 1);
    else if (character.classId === "ruhban" && character.level >= 2) nextResource = Math.min(resourceMax, nextResource + 1);
    else if (["yeminli", "ozan", "sekil-degistirici", "cagirici", "barbar"].includes(character.classId)) nextResource = Math.min(resourceMax, nextResource + 1);
    else if (character.classId === "mucit" && character.level >= 2 && !inventorRecoverySucceeded) {
      const deviceRoll = randomDie(4);
      if (deviceRoll === 4) { nextResource = Math.min(resourceMax, nextResource + 1); inventorRecoverySucceeded = true; }
      recoveryNote = ` Mucit yenilenme zarı d4 → ${deviceRoll}${deviceRoll === 4 ? ": +1 Yük." : ": Yük yenilenmedi."}`;
    }
    const nextSlots = activeRuntime.spellSlots.map((value, index) => Math.min(slotsMax[index] ?? value, value + (restDraft.slotRecovery[index] ?? 0)));
    const recoveredSlotCount = restDraft.slotRecovery.reduce((sum, value) => sum + value, 0);
    const itemUses = Object.fromEntries(Object.entries(activeRuntime.itemUses).filter(([id]) => items.find((item) => item.id === id)?.reset !== "short"));
    const nextHp = Math.min(maxHp, activeRuntime.currentHp + healing);
    const returnsFromZero = activeRuntime.currentHp === 0 && nextHp > 0 && !activeRuntime.woundCreatedThisCombat;
    const severity = returnsFromZero ? (activeRuntime.woundSeverity === "" ? "Hafif" : activeRuntime.woundSeverity === "Hafif" ? "Ağır" : "Kritik") : activeRuntime.woundSeverity;
    const woundAspect = returnsFromZero ? `${severity} Yara — ayrıntısını yaz` : activeRuntime.woundAspect;
    const result = spentDice ? `${spentDice}d${activeClass.hitDie}${signed(spentDice * conMod)} → ${healing} CP.` : "Can Zarı harcanmadı.";
    setRuntime({ ...activeRuntime, currentHp: nextHp, resource: nextResource, spellSlots: nextSlots, recoveryDice: activeRuntime.recoveryDice - spentDice, cooldowns: Object.fromEntries(Object.entries(activeRuntime.cooldowns).filter(([, type]) => type === "long")), itemUses, mageRecoveryUsed: activeRuntime.mageRecoveryUsed || recoveredSlotCount > 0, inventorRecoverySucceeded, deathSuccess: returnsFromZero ? 0 : activeRuntime.deathSuccess, deathFail: returnsFromZero ? 0 : activeRuntime.deathFail, woundSeverity: severity, woundAspect, woundCreatedThisCombat: returnsFromZero || activeRuntime.woundCreatedThisCombat, actionLog: [`Kısa mola: ${result}${recoveryNote}`, ...(returnsFromZero ? [`Yara kazanıldı: ${woundAspect}`] : []), ...activeRuntime.actionLog].slice(0, 20) });
    setRestDraft(null); setLastUse({ id: `rest-${Date.now()}`, name: "Kısa mola", result: `${result}${recoveryNote}` });
  }
  function longRest() {
    const spent = Math.max(0, character.level - activeRuntime.recoveryDice);
    const regained = spent > 0 ? Math.max(1, Math.ceil(spent / 2)) : 0;
    setRuntime({ ...activeRuntime, currentHp: maxHp, tempHp: 0, resource: resourceMax, spellSlots: [...slotsMax], rp: Math.max(2, activeRuntime.rp), deathSuccess: 0, deathFail: 0, cooldowns: {}, itemUses: {}, inCombat: false, round: 1, turn: 1, recoveryDice: Math.min(character.level, activeRuntime.recoveryDice + regained), woundCreatedThisCombat: false, mageRecoveryUsed: false, inventorRecoverySucceeded: false });
  }
  function toggleCombat() { const starting = !activeRuntime.inCombat; setRuntime({ ...activeRuntime, inCombat: starting, round: 1, turn: 1, woundCreatedThisCombat: starting ? false : activeRuntime.woundCreatedThisCombat, cooldowns: Object.fromEntries(Object.entries(activeRuntime.cooldowns).filter(([, type]) => type !== "turn")) }); }
  function endTurn() { setRuntime({ ...activeRuntime, turn: activeRuntime.turn + 1, cooldowns: Object.fromEntries(Object.entries(activeRuntime.cooldowns).filter(([, type]) => type !== "turn")) }); }
  function endRound() { setRuntime({ ...activeRuntime, round: activeRuntime.round + 1, turn: 1, cooldowns: Object.fromEntries(Object.entries(activeRuntime.cooldowns).filter(([, type]) => type !== "turn")) }); }
  function compelWound() { if (!activeRuntime.woundSeverity || activeRuntime.rp >= 5) return; setRuntime({ ...activeRuntime, rp: activeRuntime.rp + 1, actionLog: [`Yara Compel kabulü: ${activeRuntime.woundAspect} (+1 RP)`, ...activeRuntime.actionLog].slice(0, 20) }); }
  function invokeWound() { if (!activeRuntime.woundSeverity || activeRuntime.rp <= 0) return; setRuntime({ ...activeRuntime, rp: activeRuntime.rp - 1, actionLog: [`Yara Invoke: ${activeRuntime.woundAspect} (-1 RP)`, ...activeRuntime.actionLog].slice(0, 20) }); }
  function clearWound() { setRuntime({ ...activeRuntime, woundSeverity: "", woundAspect: "", actionLog: [`Yara tedavi edildi`, ...activeRuntime.actionLog].slice(0, 20) }); }
  ...
  function confirmLevelUp() {
    ...
    const snapshot: LevelSnapshot = { level: character.level, abilities: { ...character.abilities }, currentHp: activeRuntime.currentHp, maxHpValue: maxHp, resource: activeRuntime.resource, spellSlots: [...activeRuntime.spellSlots], asiLevels: [...activeRuntime.asiLevels], recoveryDice: activeRuntime.recoveryDice };
  ...
  function useAction(action: ActionSource) {
    ...
    if (action.kind === "Büyü" || (action.kind === "Yetenek" && character.classId === "buyucu")) {
      const minimum = action.kind === "Büyü" ? Math.max(0, (action.slotLevel ?? 1) - 1) : Math.max(0, tierFromText(action.tier ?? "1") - 1); const slot = nextSlots.findIndex((value, index) => index >= minimum && value > 0);
  ...
  const actions: ActionSource[] = [
    ...selectedLoadoutPowers.map((power) => ({ id: power.id, name: power.name, effect: power.effect, kind: (character.classId === "buyucu" ? "Büyü" : character.classId === "ruhban" ? "Dua" : character.classId === "mucit" ? "Cihaz" : "Teknik") as ActionSource["kind"], cost: power.cost, slotLevel: power.slotLevel, cooldown: power.cooldown ?? "none" })),
  ...
  return (
    <div className="dashboard-page shell">
      ...
      {tab === "masa" && <div className="table-layout"><main className="table-main">
        ...
        <section className="vital-grid"><Tracker icon={Heart} label="Can Puanı" value={activeRuntime.currentHp} suffix={`/ ${maxHp}`} onMinus={() => adjust("currentHp", -1, 0, maxHp)} onPlus={() => adjust("currentHp", 1, 0, maxHp)} danger={activeRuntime.currentHp <= maxHp / 4} /><Tracker icon={Shield} label="Geçici CP" value={activeRuntime.tempHp} onMinus={() => adjust("tempHp", -1)} onPlus={() => adjust("tempHp", 1)} /><Tracker icon={Sparkles} label="Rün Puanı" value={activeRuntime.rp} suffix="/ 5" onMinus={() => adjust("rp", -1, 0, 5)} onPlus={() => adjust("rp", 1, 0, 5)} /><Tracker icon={CircleDot} label={activeClass.resource} value={activeRuntime.resource} suffix={`/ ${resourceMax}`} onMinus={() => adjust("resource", -1, 0, resourceMax)} onPlus={() => adjust("resource", 1, 0, resourceMax)} /><Tracker icon={Coins} label="Altın" value={activeRuntime.gold} onMinus={() => adjust("gold", -1)} onPlus={() => adjust("gold", 1)} /><Tracker icon={Dices} label="Can Zarları" value={activeRuntime.recoveryDice} suffix={`/ ${character.level}`} onMinus={() => setRun("recoveryDice", Math.max(0, activeRuntime.recoveryDice - 1))} onPlus={() => setRun("recoveryDice", Math.min(character.level, activeRuntime.recoveryDice + 1))} /></section>
        <section className={`action-console table-card ${activeRuntime.inCombat ? "combat-live" : ""}`}><div className="action-console-head"><div><p className="eyebrow">Hızlı kullanım çubuğu</p><h2>Aktif Yetenekler</h2><span>{actions.length} hazırlanmış eylem</span></div><div className="combat-controls"><button className={activeRuntime.inCombat ? "active" : ""} onClick={toggleCombat}><Swords size={14} />{activeRuntime.inCombat ? `Çatışma · Raunt ${activeRuntime.round} · Tur ${activeRuntime.turn}` : "Çatışmayı başlat"}</button>{activeRuntime.inCombat && <><button onClick={endTurn}><RotateCcw size={14} /> Turu bitir</button><button onClick={endRound}><CircleDot size={14} /> Rauntu bitir</button></>}</div></div>{actions.length ? <div className="action-grid">{actions.map((action) => <ActionCard key={action.id} action={action} cooldown={activeRuntime.cooldowns[action.id]} onUse={() => useAction(action)} />)}</div> : <div className="action-empty"><Zap size={30} /><div><b>Hızlı çubuk boş.</b><p>Yetenek Ağacı’ndan öğrendiğin güçleri hazırla veya Envanter’den bir eşya kuşan.</p></div></div>}</section>
